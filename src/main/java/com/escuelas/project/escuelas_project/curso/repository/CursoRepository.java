...
public interface CursoRepository extends JpaRepository<Curso, UUID> {
    ...
    @Query("SELECT new com.escuelas.project.escuelas_project.curso.dtos.CursoResponseDto(c) FROM Curso c WHERE c.activo = true")
    public Optional<CursoResponseDto> findByNombreDto(String nombre);

    @Query("SELECT new com.escuelas.project.escuelas_project.curso.dtos.CursoResponseDto(c) FROM Curso c WHERE YEAR(c.fecha_de_curso) = ?1 AND c.escuela = ?2 AND c.activo = true")
    ...
    @Query("SELECT new com.escuelas.project.escuelas_project.curso.dtos.CursoResponseDto(c) FROM Curso c WHERE c.activo = true AND c.id = ?1")
    public Optional<CursoResponseDto> findByIdActiveDto(UUID id);
}
